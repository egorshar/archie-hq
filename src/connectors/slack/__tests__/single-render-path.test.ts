...
describe('an attachment-only bot post survives into every agent-facing entry point', () => {
  ...
  it('reaches the title generator — the transcript it sends to the one-shot carries the report headline', async () => {
    const { thread } = await fetchFixtureThread();
    // Asserted on the transcript rather than on the LLM call: this fork routes the
    // one-shot through `getLlmOneShot()` (so there is no SDK prompt to inspect), and
    // the transcript IS what `generateTitle` sends — `applyGeneratedTitle` passes it
    // through unchanged. The render path under test ends here.
    const { buildSlackTitleTranscript } = await import('../title-transcript.js');

    const { transcript } = buildSlackTitleTranscript(thread);

    expect(transcript).toContain(PHRASE);
  });
