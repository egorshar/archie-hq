...
export function TaskDetail({ taskId, onBack, liveEvents, onConnect }: TaskDetailProps) {
  ...
  const logLines: { node?: React.ReactNode; approval?: { approvalType: 'edit_mode' | 'research_budget' | 'merge' | 'trigger' | 'max_mode' | 'tool_call'; eventIndex: number; github?: string; pr_number?: number; ref?: string }; fold?: { id: string; summary: React.ReactNode; full: React.ReactNode } }[] = [];
