...
beforeEach(() => { for (const k of ['OPENROUTER_API_KEY', 'SLACK_BOT_TOKEN']) SAVED[k] = process.env[k];
  process.env.OPENROUTER_API_KEY = 'sk-or'; process.env.SLACK_BOT_TOKEN = 'xoxb'; });
...
describe('repoHostEgressDomains', () => {
  it('a repo agent gets the fixed github egress hosts', () => {
    const p = buildChildSandboxProfile({ agent: agent(), task, cwd: '/clone', editAllowed: false, maxMode: false, proxy: fakeProxy() });
    expect(p.allowlist).toEqual(expect.arrayContaining(['github.com', 'api.github.com', 'codeload.github.com']));
  });
  it('a clone-less agent (no repo) gets NO git host in the allowlist', () => {
    const synthetic = agent({ def: { id: 'pm-agent', repo: undefined }, sandbox: { cwd: '/synthetic', allowReadPaths: [], allowWritePaths: [], denyWritePaths: [] } });
    const p = buildChildSandboxProfile({ agent: synthetic, task, cwd: '/synthetic', editAllowed: false, maxMode: false, proxy: fakeProxy() });
