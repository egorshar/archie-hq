...
// The refusals this fork actually emits. Both name `target.new_dm` /
// `target.new_thread`, which the fork added and upstream has no equivalent for —
// so the wording differs from upstream's while the assertion's point stands:
// with no home channel, the home-channel exception changes nothing.
