 ...
 * Backend Agent
 *
 * Senior Ruby on Rails engineer responsible for the backend repository.
 * Can coordinate with Mobile agent for cross-repo issues.
...
/**
 * Generate the system prompt for the Backend agent
 */
function getBackendSystemPrompt(isTaskOwner: boolean): string {
...
IMPORTANT: After sending your findings to the task owner, STOP and wait for further instructions.`;
  ...
  return `You are the Backend Agent, a senior Ruby on Rails engineer responsible for the backend repository.

Available peer agents:
- mobile-agent: Manages mobile app (iOS/Android, React Native)
...
4. If the issue involves mobile code, coordinate with mobile-agent
...
/**
 * Spawn a Backend agent with streaming input from a message queue
 * Returns an AgentHandle to track the running agent
 */
export async function spawnBackendAgent(
  ...
  agentName: string = 'backend-agent'
): Promise<AgentHandle> {
  const repoPath = metadata.repositories.backend?.path || '/repos/backend';
...
`;
  ...
  const systemPrompt = getBackendSystemPrompt(isTaskOwner);
