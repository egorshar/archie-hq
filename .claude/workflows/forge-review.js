...
const wt = `${scratch}/pr-${input.pr}`
const mutWt = `${scratch}/pr-${input.pr}-mutation`
...
if (!setup || !setup.ok) {
  await teardown()
  return { status: 'error', reason: `could not prepare a worktree for PR #${input.pr}: ${setup ? setup.notes : 'setup agent failed'}` }
}
...
const lenses = [
  ...
  { key: 'diff-mapper', prompt: `You map what a diff ACTUALLY does, function by function. Work read-only in ${wt}; the diff: ${diffCmd}. Report as claims: each behavioral change the diff makes, where it diverges from or exceeds what a reader of the PR title alone would expect, new tests and what they assert, and anything unfinished (TODOs, dead code, unreferenced additions). ${CONTRACT}${followUp}` },
  { key: 'drift-check', prompt: `You check base-branch drift for PR #${input.pr}. Work read-only in ${wt}: what has landed on ${setup.base} since this PR branched (git log/diff against the merge-base) that touches the same files or subsystems? Report as claims: likely conflicts, invalidated assumptions, or duplicated work. ${CONTRACT}` },
]
const checked = await pipeline(
  ...
  (found, lens) => {
    ...
    return agent(
      `You are an adversarial fact-checker with read-only access to the repo (worktree: ${wt}) and the PR (GitHub MCP via ToolSearch / gh). ${GH_READONLY} For EACH claim below, try to REFUTE it against the actual code or source. Verdict per claim: CONFIRMED / WRONG (state what is actually there) / UNVERIFIABLE. Facts only. Claims:\n${JSON.stringify(found.claims, null, 2)}`,
...
return { status: 'ok', pr: input.pr, headSha: setup.headSha, intent: derived.intent, assumptions: derived.assumptions, acs: derived.acs, reviewFindings, previousFindingRulings, qaManifest, gaps, recommendation }
