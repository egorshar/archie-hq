...
1. Record the plan: write docs/plans/$(date +%Y%m%d)-${input.change || 'change'}.md per that folder's README convention — a status header ("Status: Implemented (this PR)"), then the brief, the acceptance criteria with their methods, the design, the tasks, and the verification plan. Add a row for it to the Implementation Status table in docs/plans/README.md. Never hard-wrap prose. Commit it on ${input.branch}.
