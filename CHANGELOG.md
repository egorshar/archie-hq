...
- **Edit-mode approval now makes a repo agent writable even when the user approves while that agent is still starting up.** PR #191 restarts repo agents that are *running* at approval so they re-mount read-write, but an agent still mid-boot at that instant — no live process handle yet — slipped past its `isRunning` check, finished booting read-only, and then hit "permission denied" on every git write: the exact "approve while the engineer is already working" race that motivated #191. `Task.ensureAgentSpawned` now reconciles it the next time work is delivered — a repo agent that booted read-only while edit mode is approved is torn down and re-spawned writable, resuming the same session. _Technical: new `Agent.editModeAtSpawn` records the `edit_allowed` a process was built under (its sandbox mount and repo-tool allowlist are frozen from it at spawn time); the reconcile reuses the established abort + `queue.stop()` + drop + fresh-`Agent` teardown rather than an in-place re-spawn (which would trip the aborted boot's session-recovery on a still-live shared queue). Live-verified against a booted instance: approving mid-boot triggers the reconcile and `create_branch` + commit then succeed, with no read-only-filesystem denials._
