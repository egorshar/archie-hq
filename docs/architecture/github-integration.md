...
| `create_branch` | Create a new branch and switch to it. Default: auto-named `archie/{taskId}` or `archie/{taskId}-N`; optionally pass `ticket` (+ `type`, `slug`) to compose a git-flow ticket branch like `feature/SWEED-123-fix-auth-flow` for repos whose push rules reject `archie/*` names. |
