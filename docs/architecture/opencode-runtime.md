...
- **Step budget (`maxTurns`)** — opencode drives a turn to `session.idle` with no ceiling of its own, so the budget is Archie's. `waitForTurn` records the agent's allowance (`def.maxTurns`, default `DEFAULT_MAX_TURNS` = 100) alongside the waiter; the SSE consumer counts `message.updated` events for **assistant** messages, one distinct message id per API round-trip, and on the step that goes over the allowance the registry rejects the waiter and the consumer calls `session.abort`. Both halves matter: rejecting alone unblocks Archie while opencode keeps driving the turn and spending tokens. A breach surfaces exactly like any other turn error — the agent is marked inactive and the task's bounded recovery loop runs — which is the Claude runtime's behaviour on `error_max_turns`. The **span differs by runtime**, because the turn primitives differ: the Claude SDK counts round-trips across the whole streamed query (the agent's lifetime in the task), while here the allowance is **per prompt**, so each brief the PM sends starts fresh. Per prompt is the useful span for a safety net — it stops a runaway investigation without leaving the agent spent for the rest of the task. Measured motivation: one research brief reached 139 round-trips and 10.5M input tokens with `maxTurns: 50` declared and silently ignored.
...
| `turn-completion.ts` | In-process registry: `waitForTurn` / `completeTurn` / `failTurn` / `cancelTurn`, plus the per-turn step budget (`noteAssistantStep`). |
