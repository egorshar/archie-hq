...
- At spawn, `spawnAgent()` injects `GIT_AUTHOR_NAME`/`GIT_AUTHOR_EMAIL` into the repo agent's process environment from that field. Git applies these to the author only; the committer keeps falling back to the `user.*` config (the bot). No agent cooperation is needed — every `git commit`, merge, and rebase the agent runs picks them up automatically.
