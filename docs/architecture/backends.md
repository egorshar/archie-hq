...
Archie's engineering path depends on two pluggable backends, each behind its own seam so an alternative can be selected without touching call sites. A single resolver in `src/system/backends.ts` picks both:

- **Agent runtime** (`AGENT_RUNTIME`) — what actually executes the coding-agent process: the Claude Agent SDK (default) or `opencode` (see `docs/architecture/opencode-runtime.md`).
- **Repo host** (`REPO_HOST`) — where pull/merge requests, reviews, and CI live: GitHub (default) or GitLab (self-hosted), both fully wired as `RepoHost` implementations.

`src/index.ts` calls `assertBackendConfig()` early in boot (validating both selections), and `getBackendMatrix()` returns `{ runtime, repoHost }` for the boot log line and the `backends` field on `GET /health`.
