...
- **`askpassToken()` wiring.** `RepoHost.askpassToken()` is implemented by both hosts and wired end-to-end into the clone/push path (B5): `scripts/git-askpass.sh` reads `REPO_HOST` and returns GitHub's `x-access-token` + App installation token or GitLab's `oauth2` + `$GITLAB_TOKEN` accordingly, and `repoCloneUrl()`/`repoEventPrefix()` (`src/connectors/shared/repo-url.ts`) derive the clone URL and knowledge-log destination prefix (`github:` / `gitlab:`) from the same env, so the base cache, shared clones, and startup clone all resolve to the correct host without a host-specific branch at each call site.
