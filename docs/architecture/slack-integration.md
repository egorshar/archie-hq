...
- **Deliver.** Each rendered line goes through a single sink, `Task.onStatusRendered`, which fans it out to three surfaces so the status can be observed with or without Slack: (1) a `status` event on the in-process event bus → SSE → the **CLI** renders the same "Archie is …" line live (a spinner above the message input — `TaskDetail.tsx`); (2) the unified `logger.slack` so it's visible in headless / dry-run / `npm run dev` output; (3) the **Slack** assistant-thread indicator (best-effort, every linked non-muted Slack thread). A CLI-only task has no Slack channel, so the Slack push is a no-op there and the CLI/log surfaces are how you test it.
