...
**Repo post-checkout hook (optional).** `ARCHIE_REPO_POSTCHECKOUT` — an operator-defined shell command run in each repo clone right after checkout, on every agent spawn, for repos whose frontmatter opts in with `postCheckout: true` (in `metadata.archie.repos[]`). It runs in the clone dir with the orchestrator environment plus `ARCHIE_POSTCHECKOUT_REPO` (the `github` id) and `ARCHIE_POSTCHECKOUT_EDIT_MODE` (`1`/`0`); best-effort (a failure or 120s timeout is warn-logged and never blocks the spawn). Typical use: write a `.npmrc` and run `npx ai-context sync` so the clone carries the right per-harness context files before the agent works in it. **Security split — intentional:** the *command* is operator/env-controlled because the hook runs in the orchestrator process, which holds every secret; plugin frontmatter supplies only the per-repo boolean (which repos it runs for), never the command string — so a hot-reloaded plugins change can't introduce orchestrator code execution. If the command writes files that shouldn't be committed (the synced context files, `.npmrc`), have it append them to the clone's `.git/info/exclude`.
...
- **Outbound:** GitHub, Anthropic API, Slack API (all trusted). Socket Mode also relies on a long-lived outbound WebSocket to `wss-primary.slack.com`. Under `AGENT_RUNTIME=opencode` the set also includes the configured model provider host(s) (e.g. OpenRouter), the git host (GitLab base URL or GitHub), and the trusted package registries (npm/Yarn) for edit-mode repo work.
- **Egress allowlist firewall (recommended posture — opencode runtime).** Run the host with a default-deny egress firewall (nftables, or a cloud security group) that permits only the outbound destinations above. This is the kernel/network-enforced backstop for the opencode per-child sandbox: that sandbox's cooperative egress proxy filters only the serve process's own `fetch` (model/`webfetch`/remote-MCP) and cannot see `bash`-spawned subprocess traffic (`git`/`npm`/`curl`), so the host firewall is what coarse-filters that traffic at the network layer. The two are complementary — the per-agent proxy adds granularity (per-agent allowlists, per-child credentials, logging) on top of the firewall's host-wide backstop. This is a compensating control implemented as deployment configuration, not something the codebase enforces; details of what the in-process sandbox does and doesn't cover are in [opencode Setup — Honest boundary](opencode-setup.md#per-child-os-sandbox-p3b).
