...
- **Skill hot-reload needs a process restart to take effect.** A plugins push re-stages `<serveRoot>/.opencode/skills/` (so the disk is correct) and the change is picked up when the process next restarts — but the running embedded server does NOT reflect it live: opencode freezes the discovered skill set at serve startup (verified CASE 2, CLI `1.17.16`; spike: `runtime/opencode/__spike__/skills-refresh-spike.md`). Concretely, until a restart: a modified skill serves stale content, a newly added skill is invisible (load → "not found"), and a removed skill stays listed but fails to load. Making a hot-reload take effect live would require a managed serve restart (drain in-flight turns → restart the serve child → re-establish sessions via the recovery path); that is a deferred follow-up, not implemented. The disk re-stage is retained because it makes the next restart correct with no extra work.
