...
**Session store.** opencode persists conversations in a SQLite store selected by `XDG_DATA_HOME` (`<data-dir>/opencode/opencode.db`). Left unset, that resolves to the real user's `~/.local/share/opencode` — a single process-global store shared across every agent and, in a container, an ephemeral path wiped on each redeploy. Archie instead pins `HOME`/`XDG_DATA_HOME` per agent to `<workdir>/opencode-server/<taskId>/<agentId>/home`, which changes three things:

- **Location + redeploy survival.** The store lives on the `workdir` volume (the same mounted volume as clones and sessions), so it outlives the `opencode serve` child AND the Archie daemon: a child recycle/reap or a full daemon redeploy resumes the agent's sessions with prior context rather than losing them. A missing or erased store is not fatal — a resume miss falls back to the runtime's stale-session (`NotFoundError`) recovery, which creates a fresh session and retries. (This is why a task reopened *after* its serve root was evicted cold-starts cleanly.)
- **Per-agent isolation (threat note).** Each agent's store is its own directory, so one agent's serve child cannot resume another agent's sessions — the shared-store cross-agent read that a single global store would allow is closed by construction. (The bridge's per-child tokens authenticate *tool calls*; the per-agent data dir is what isolates *session storage*.)
- **Growth + housekeeping.** Session rows accumulate per turn and Archie does not prune them; the whole per-agent store is removed with the task's serve root at teardown (`evictTask`), so growth is bounded per task rather than unbounded process-wide. A scripted single-session smoke leaves an `opencode.db` of only a few hundred KB, so per-task accumulation is not a near-term concern; a TTL/scheduled cleanup is a deferred follow-up, and the per-agent-dir layout already makes cleanup structural (the store is `rm`'d with the child's task).
...
- **Idle children accumulate memory over a long-lived task.** An idle `opencode serve` child is ~300 MB RSS; `OPENCODE_CHILD_IDLE_TTL` (default 15m) reaps a parked agent's child (context-free respawn — the agent's session store is pinned to its per-agent data dir on the workdir volume, so a reaped child's next boot resumes from it; see "Session store" below), and `OPENCODE_CHILD_SOFT_CAP` (default 12) only emits a census warning when live children exceed it — it never queues or blocks a spawn, so a burst of concurrent agents can still transiently exceed the soft cap.
