...
Prompt parity: agent prompts are shared across runtimes via per-runtime variables (`src/agents/prompt-runtime-vars.ts`) — tool names are lowercased (`Read` → `read`, etc.) and the Claude render is byte-identical. On skills, opencode **does** have a native, Claude-compatible skill tool (`skill`, which discovers `.claude/skills/<name>/SKILL.md`), and Archie already links each agent's domain skills in that format under `<agentWorkspace>/.claude/skills/`. They are not wired to opencode yet, though: the embedded server's cwd is the Archie repo root and the runtime does not point the session at the per-agent workspace, so opencode's skill discovery wouldn't see the agent's skills. Until that's wired, the opencode prompts route agents to the equivalent in-context domain guidance (AGENTS.md + task briefing) instead of the `skill` tool. See Known limitations.
...
- **Native skills not wired yet.** opencode has a Claude-compatible `skill` tool that loads `.claude/skills/<name>/SKILL.md`, and Archie already emits skills in that format per agent — but the opencode session isn't pointed at the per-agent workspace, so discovery wouldn't find them. The prompts therefore route opencode agents to in-context domain guidance instead. Wiring native skills (session `directory` → the agent workspace, un-neutralized prompts referencing the `skill` tool, and `capabilities.skills = true`) is a tracked follow-up that would restore skill parity with the Claude runtime.
