...
Prompt parity: agent prompts are shared across runtimes via per-runtime variables (`src/agents/prompt-runtime-vars.ts`) — tool names are lowercased (`Read` → `read`, etc.) and the Claude render is byte-identical. On skills, opencode's current releases have a native, Claude-compatible `skill` tool (it discovers `.claude/skills/<name>/SKILL.md`), and Archie already links each agent's domain skills in that format under `<agentWorkspace>/.claude/skills/`. But the pinned `@opencode-ai/sdk` (1.17.16) predates that feature — its `Config` has no skill key, and a live probe against the CLI confirmed no `skill` tool surfaces (even with the session pointed at a workspace containing `.claude/skills/`). So the opencode prompts route agents to the equivalent in-context domain guidance (AGENTS.md + task briefing) instead. See Known limitations.
...
- **Native skills need an opencode upgrade first.** opencode's current releases have a Claude-compatible `skill` tool that loads `.claude/skills/<name>/SKILL.md` (Archie already emits skills in that format per agent), but the pinned `@opencode-ai/sdk` 1.17.16 predates it (no skill `Config` key; a live probe surfaced no `skill` tool). Restoring skill parity is a follow-up that starts with bumping opencode to a skills-capable release, and then resolving how per-agent skills map onto the shared embedded server (opencode discovers skills by the server's working directory, not per-session — so a shared server would need a shared/global skills set or per-agent servers). Until then, opencode agents use in-context domain guidance.
