...
Prompt parity: agent prompts are shared across runtimes via per-runtime variables (`src/agents/prompt-runtime-vars.ts`) — tool names are lowercased (`Read` → `read`, etc.) and the Claude render is byte-identical. On skills, the pinned opencode (1.17.16) DOES have a native `skill` tool, and it discovers both `.opencode/skills/<name>/SKILL.md` and Claude-compatible `.claude/skills/<name>/SKILL.md` (confirmed by a live probe). Archie already links each agent's domain skills in that format. They aren't wired to opencode yet because of a discovery-model mismatch, not the version: opencode discovers skills by the **serve process's working directory at startup** (it injects the skill list into the session prompt) — NOT per-session (a probe pointing `query.directory` at a skills workspace surfaced nothing). Archie runs ONE shared embedded server (cwd = repo root) for all agents, each with its own per-task skill set, so serve-cwd discovery can't hand each agent its own skills. Until that's resolved the opencode prompts route agents to the equivalent in-context domain guidance (AGENTS.md + task briefing). See Known limitations.
...
- **Native skills not wired (discovery-model mismatch, not version).** The pinned opencode 1.17.16 already supports skills — a live probe confirmed the `skill` tool discovers both `.opencode/skills/` and `.claude/skills/` SKILL.md files, and Archie already emits skills per agent in that format. The gap is that opencode discovers skills by the **serve process's working directory at startup**, while Archie runs one shared embedded server (cwd = repo root) for all agents with per-task skill sets, and it overrides the session system prompt (where opencode injects the skill list). Restoring skill parity therefore needs a real design decision — a shared/global skills set discoverable at the serve cwd (every agent sees every skill), or per-agent serve processes — plus confirming the injected skill list survives Archie's `body.system` override. Until then, opencode agents use in-context domain guidance.
