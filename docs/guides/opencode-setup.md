...
- **No OS sandbox.** Unlike the Claude runtime, the opencode runtime has no kernel-level sandbox: read-only mode is guard-enforced (the plugin `before` hook + bridge dispatch), not enforced by the OS, and agent bash/egress are unsandboxed. A per-child sandbox (bubblewrap mounts + an egress-filtering proxy) is planned future work.
